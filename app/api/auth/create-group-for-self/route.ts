...
export async function POST(req: Request) {
  ...
  if (!profile) return NextResponse.json({ error: 'User not found — please sign in again' }, { status: 404 })

  const subdomain = (groupNameEn.trim() + managerNameEn.trim())
    .toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 40)
  if (!subdomain) return NextResponse.json({ error: 'English name must contain letters or numbers' }, { status: 400 })

  const { data: existing } = await supabase
    .from('groups').select('id').eq('subdomain', subdomain).maybeSingle()
  if (existing) return NextResponse.json({ error: `Path "${subdomain}" is taken — try a different English name`, subdomain }, { status: 400 })
  ...
  if (memberError) {
    return NextResponse.json({ error: 'Workspace created but failed to set admin: ' + memberError.message }, { status: 400 })
  }
